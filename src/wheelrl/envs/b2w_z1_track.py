...
# The Z1 workspace is wide (measured grid: x in [-0.65, 1.11], y in [-0.56,
# 0.56], z in [-0.26, 1.09]); the tracking target is confined to the working
# zone around the nominal EE pose [0.66, 0.01, 0.67] so the arm stays in a
# comfortable region and the base is not forced to over-drive.
EE_POS_BOUNDS = np.array(
    [[0.30, 1.00], [-0.45, 0.45], [0.25, 1.00]], dtype=np.float64
)
